...
import { SSLCommerzInitRequest } from "../types/sslcommerz";
...
export class DonationController {
  async createDonationSession(req: Request, res: Response) {
    try {
      ...
      const transactionId = `DON-${uuidv4()}`;
      ...
      const sslRequest: SSLCommerzInitRequest = {
        total_amount: amount,
        currency: "BDT",
        tran_id: transactionId,
        // success_url: `${config.baseUrl}/api/donations/success/${transactionId}`,
        success_url: `${config.baseUrl}/api/donations/success?transactionId=${transactionId}`,

        fail_url: `${config.baseUrl}/api/donations/fail/${transactionId}`,
        cancel_url: `${config.baseUrl}/api/donations/cancel/${transactionId}`,
        ipn_url: `${config.baseUrl}/api/donations/ipn`,
        cus_name: name,
        cus_email: email,
        cus_add1: "N/A",
        cus_city: "N/A",
      ...
      const sslResponse = await sslCommerzService.initTransaction(sslRequest);
  ...
  async handleSuccess(req: Request, res: Response) {
    let transactionId: string | undefined; // transactionId কে try ব্লকের বাইরে ডিফাইন করা

    try {
      // req.query থেকে ডিস্ট্রাকচার করা
      ({ transactionId } = req.query as { transactionId?: string });
      ...
      if (!transactionId) {
        ...
        return res.redirect(
          `${config.frontendUrl}/donation/failed/error=missing_transaction_id`
        );
      }
      ...
      if (!donation) {
        ...
        return res.redirect(
          `${config.frontendUrl}/donation/failed/transactionId=${transactionId}&error=donation_not_found`
        );
      }
      ...
      console.error("handleSuccess error:", error);
      return res.redirect(
        `${config.frontendUrl}/donation/failed/transactionId=${
  ...
  async handleFailure(req: Request, res: Response) {
    try {
      ...
      if (!transactionId) {
        ...
        return res.redirect(`${config.frontendUrl}/donation/failed/unknown`);
      }
      ...
      console.error("Error handling failure:", error);
      return res.redirect(`${config.frontendUrl}/donation/failed/error`);
    }
  }
  ...
  async handleCancel(req: Request, res: Response) {
    try {
      ...
      return res.redirect(
        `${config.frontendUrl}/donation/canceled/${transactionId}`
      );
    } catch (error) {
      console.error("Error handling cancel:", error);
      return res.redirect(`${config.frontendUrl}/donation/canceled/unknown`);
    }
  }
  ...
  async handleIPN(req: Request, res: Response) {
    try {
      ...
      const donation = await DonationModel.findOne({ transactionId: tran_id });
